...
if __name__=="__main__":
    ...
    loc=col1.selectbox("Select Location : ", list(Location.keys()))
    facing=col2.selectbox("Select Facing : ", list(Facing.keys()))
    Types=col2.selectbox("Select Type : ", list(Type.keys()))
    result=predict(bed,bath,loc,size,status,facing,Types)
    submit_button=st.button("predict")
    if submit_button:
        larger_text=f"<h2> The predicted price is {result} Lakhs</h2>"
